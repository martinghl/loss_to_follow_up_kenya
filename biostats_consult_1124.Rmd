...
# Plot for Model 1
ggplot(tidy_model1, aes(x = reorder(term, estimate), y = estimate, ymin = lower, ymax = upper)) +
  geom_point(aes(color = significance), size = 2) +
  geom_errorbar(width = 0.2, aes(color = significance)) +
  coord_flip() +
  scale_y_continuous(limits = c(-5, 5)) +  # Adjust axis limits as needed
  labs(
    title = "Model 1",
    x = "Predictors",
    y = "Estimate"
...
# Plot for Model 2
ggplot(tidy_model2, aes(x = reorder(term, estimate), y = estimate, ymin = lower, ymax = upper)) +
  geom_point(aes(color = significance), size = 2) +
  geom_errorbar(width = 0.2, aes(color = significance)) +
  coord_flip() +
  scale_y_continuous(limits = c(-5, 5)) +  # Adjust axis limits as needed
...
```{r}
model1_tidy <- tidy(logistic_followup_model1) %>%
  select(term, estimate, std.error, p.value) %>%
  arrange(p.value)

model2_tidy <- tidy(logistic_followup_model2) %>%
  select(term, estimate, std.error, p.value) %>%
  arrange(p.value)
```

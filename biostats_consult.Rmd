...
# summary(data_2022_cleaned$age)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  17.44   32.26   38.03   38.76   44.70   83.64 
...
# 确保term在数据中存在且与term_order匹配, 去除多余水平
...
# 如果需要保存结果数据为CSV
write.csv(combined, "S:\\Downloads\\combined_results.csv", row.names = FALSE)
...
# 假设你的数据集是 completed_data_2022，并且 if_follow 和 if_follow2 都是0/1变量
df_table <- data.frame(
...
df_mca <- completed_data_2022 %>%
  select(highest_education, employment_status, household_income,
         residence_area, gender, marital_status, religion, age_category)

sapply(df_mca, nlevels)
...
fviz_mca_biplot(res.mca, repel = TRUE) +
  labs(title = "MCA Biplot for Nominal Variables")

fviz_mca_var(res.mca, choice = "mca.cor", repel = TRUE) +
  labs(title = "MCA - Variables Representation")

fviz_mca_biplot(res.mca, repel = TRUE) +
  labs(title = "MCA - Categories Representation")
